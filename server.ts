...
async function startServer() {
  ...
  app.get("/api/crm/state", async (_req, res) => {
    try {
      ...
      console.error("[CRM State Read Error]", error);
      res.status(500).json({
        error: "CRM_STATE_READ_FAILED",
        message: error.message || "Nao foi possivel carregar os dados do CRM.",
      });
    }
  });
  ...
  app.post("/api/crm/reset", async (_req, res) => {
    try {
      const state = await resetCRMState();
      res.json(state);
    } catch (error: any) {
      console.error("[CRM State Reset Error]", error);
      res.status(500).json({
        error: "CRM_STATE_RESET_FAILED",
        message: error.message || "Nao foi possivel resetar os dados do CRM.",
      });
    }
  });

  const sendError = (res: express.Response, error: any, fallback: string) => {
    console.error("[CRM Entity API Error]", error);
    res.status(500).json({
      error: "CRM_ENTITY_API_FAILED",
